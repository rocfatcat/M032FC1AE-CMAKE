...
IROM2_SECTION int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    else if (lcmd == CMD_RUN_APROM || lcmd == CMD_RUN_LDROM || lcmd == CMD_RESET)
    {
        ...
        /* Set BS */
        if ((lcmd == CMD_RUN_APROM) || ((FMC->ICPCTL & FMC_ISPCTL_BS_Msk) == 0))
        {
        ...
        {
            /* Change vector page address to LDROM */
            FMC_SetVectorPageAddr(FMC_LDROM_BASE);
            /* Point to the reset handler address which bootloader uses */
            func = (FUNC_PTR *) * (uint32_t *)(FMC_LDROM_BASE + 4);
            /* Set stack pointer base address to the one which bootloader uses */
